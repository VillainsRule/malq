import LWUpdate from '../../util/livewire/LWUpdate';

import Provider, { type Mail } from '../Provider';

export default class priyo$email extends Provider {
    livewire: LWUpdate = new LWUpdate('priyo.email');

    async getAddress(): Promise<string> {
        await this.livewire.pullHTML();

        const email = this.livewire.html.match(/const email = '(.*?)'/)?.[1];
        if (!email) throw new Error('[priyo.email] (livewire) email not found in HTML');

        this.livewire.dispatch('themes.components.account-actions', 'syncEmail', { email });
        this.livewire.dispatch('themes.components.action', 'syncEmail', { email });
        this.livewire.dispatch('themes.components.inbox-message', 'syncEmail', { email });
        this.livewire.dispatch('themes.components.inbox-message', 'fetchMessages');
        await this.livewire.sendoff();

        this.address = email;
        return email;
    }

    async getMail(): Promise<Mail[]> {
        this.livewire.dispatch('themes.components.inbox-message', 'fetchMessages');

        const res2 = await this.livewire.sendoff();
        const snapshot = JSON.parse(res2.components[0].snapshot);
        const messages = snapshot.data.messages[0].map((e: any) => e[0]);

        return messages.map((msg: any) => ({
            from: msg.sender_email,
            to: this.address!,
            subject: msg.subject,
            body: msg.content,
            date: new Date(msg.date).getTime()
        }));
    }
}