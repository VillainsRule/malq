...
export default class mailporary$com extends Provider {
    ...
    async getAddress(): Promise<string> {
        const req = await this.fetch('https://mailporary.com');
        const res = await req.text();

        const domains = res.match(/emailDomains:"\[(.*?)\]"/)?.[1].split(',').map((e) => e.replace(/\\"/g, '').trim()) || [];
        const email = `${getRandomName()}@${domains[Math.floor(Math.random() * domains.length)]}`;
    ...
    async getMail(): Promise<Mail[]> {
        const req = await this.fetch(`https://web.mailporary.com/api/v1/mailbox/${encodeURIComponent(this.address)}`, {
        ...
        const finalMail: Mail[] = await Promise.all(returnableMail.map(async (e) => {
            if (!e.body && e.id) await this.fetch(`https://web.mailporary.com/api/v1/mailbox/${encodeURIComponent(this.address)}/${e.id}`, {
                headers: { Authorization: `Bearer ${this.token}`, ...this.getHeaders() }
            }).then(async (bodyReq) => {
