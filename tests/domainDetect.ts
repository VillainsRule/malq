...
tooFew.forEach(async (d) => {
    ...
    try {
        ...
        if (req.status >= 200 && req.status < 300) {
            ...
            if (body.includes('function cookieExists(name) {')) console.log(`${root}: laravel (old)`);
            else if (body.includes('limit_error = ')) console.log(`${root}: laravel (new)`);
            ...
            else if (body.includes('Just a moment...')) console.log(`${root}: UAM`);
            else if ([
                'parked',
                '#101c36',
                '#1a1f2e',
            ...
            ].some(e => body.includes(e.toLowerCase()))) console.log(`${root}: parked`);
            else if (body.includes('Attention Required!')) console.log(`${root}: WAF`);
            else if (body.includes('/jschallenge/')) console.log(`${root}: jschallenge`);
            else if (body.includes('recaptchadiv')) console.log(`${root}: recapwaf`);
            else if (body.includes('One moment, please...')) console.log(`${root}: infinitywaf`);
        } else console.log(`${root}: ${req.status}`);
    } catch (err: any) {
        if (err.name === 'TimeoutError') console.log(`${root}: timeout`);
